...
std::string describe(CLightRef tl) {
    ...
    std::string s;
    std::string duration = std::to_string(tl.duration_sec);
    std::string pedestrian = tl.pedestrian_mode ? "true" : "false";
    s = "state: " + to_string(tl.state) + ", duration_sec: " + duration + ", pedestrianMode: " + pedestrian;
    return s;
}
