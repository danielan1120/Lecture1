...
public:
    ...
    {
        return 0.5 * base_skill * (1 - fatigue_level);
    }

    virtual float tackleSuccessRate(int base_skill)
    {
        return base_skill * (1.0f - (0.5f * fatigue_level));
    }
    ...
    {
        float effective_stamina = base_stamina * (1 - fatigue_level);
...
public:
    ...
    float shootingSuccessRate (int base_rate) override
    {
        float origin_rate = Player::shootingSuccessRate(base_rate);
        float specialized_rate = origin_rate * 0.8;
        std::cout << "Success rate -- Shooting (Defender): " << std::fixed << std::setprecision(2) << specialized_rate << "%" << std::endl;
        
        return specialized_rate;
    }
    
    float tackleSuccessRate (int base_rate) override
    {
        float origin_rate = Player::tackleSuccessRate(base_rate);
        float specialized_rate = origin_rate * 1.5;
...
public:
    ...
    {
        float origin_rate = Player::shootingSuccessRate(base_rate);
        float specialized_rate = origin_rate * 2;
    ...
    {
        float origin_rate = Player::tackleSuccessRate(base_rate);
        float specialized_rate = origin_rate * 0.5;
...
int main() {
    ...
    Defender* def = new Defender("Minjae Kim");
    def->checkEnergy(base_stamina_value);
    def->checkEnergy(base_hydration_value);
    def->tackleSuccessRate(base_skill_value);
    ...
    Striker* sonny = new Striker("Heungmin Son");
    sonny->checkEnergy(base_stamina_value);
    sonny->checkEnergy(base_hydration_value);
    sonny->shootingSuccessRate(base_skill_value);
